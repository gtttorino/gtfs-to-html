  ...
  const filteredCalendarDates = calendarDates.reduce((memo, calendarDate) => {
    if (moment(calendarDate.date, 'YYYYMMDD').isBetween(start, end)) {
      if (calendarDate.exception_type === 1) {
        if (memo.includedDates.indexOf(formatters.formatDate(calendarDate, config.dateFormat)) === -1)
        {
          memo.includedDates.push(formatters.formatDate(calendarDate, config.dateFormat));
        }
      } else if (calendarDate.exception_type === 2) {
        if (memo.excludedDates.indexOf(formatters.formatDate(calendarDate, config.dateFormat)) === -1)
        {
...
const getStopOrder = async (timetable, config) => {
  ...
  try {
    ...
    for (const trip of timetable.orderedTrips) {
      ...
      // If `showOnlyTimepoint` is true, then filter out all non-timepoints
      if (config.showOnlyTimepoint === true) {
        sortedStopIds = trip.stoptimes.filter(stoptime => isTimepoint(stoptime)).map(stoptime => stoptime.stop_id);
      } else {
        sortedStopIds = trip.stoptimes.map(stoptime => stoptime.stop_id);
      }
...
exports.getFormattedTimetablePage = async (timetablePageId, config) => {
  ...
  timetablePage.route_url = timetableRoutes.map(route => route.route_url);
  ...
  timetablePage.directionNames = uniq(timetablePage.timetables.map(timetable => timetable.direction_name));
